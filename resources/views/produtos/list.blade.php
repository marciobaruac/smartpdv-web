							...
							<div class="card-header">
									...
									<div class="dropdown dropdown-inline" data-toggle="tooltip" title="" data-placement="left" data-original-title="Ações">
										...
										<div class="dropdown-menu p-0 m-0 dropdown-menu-md dropdown-menu-right">
											...
											<ul class="navi navi-hover">
												...
												@if($p->estoque)
												   @if($p->estoque->quantidade <= 0)
												      <li class="navi-item">
												         <a onclick='swal("Atenção!", "Deseja inativar este produto?", "warning").then((sim) => {if(sim){ location.href="/produtos/delete/{{ $p->id }}" }else{return false} })' href="#!" class="navi-link">
													        <span class="navi-text">
															  <span class="label label-xl label-inline label-light-danger">Inativar Produto</span>
														    </span>
													      </a>
											     	   </li>
													@endif
								...
								<div class="card-body">
									...
									<div class="kt-widget__info">
										...
										<a class="kt-widget__data text-success">{{number_format($p->valorTabelaPadrao(), 2, ',', '.') }}</a>
								...
								<!--	<div class="kt-widget__info">
										...
										<a class="kt-widget__data text-success">
											{{number_format(($p->valorMediaDeCompra()/$p->conversao_unitaria), 2, ',', '.')}}
										</a>
									</div> -->
									...
									<div class="kt-widget__info">
										...
										<a class="kt-widget__data text-success">
											@if($p->composto==0)
											    {{number_format($p->valor_compra, 2, ',', '.') }}
											@else
											   {{number_format($p->custoprodutofabricado($p->id), 2, ',', '.') }}
									...
									<div class="kt-widget__info">
										<span class="kt-widget__label">Lucro Bruto (%):</span>
										<a class="kt-widget__data text-success">
										   {{number_format($p->lucro_bruto, 2, ',', '.') }}
										</a>
									</div>
									...
									<div class="kt-widget__info">
										<span class="kt-widget__label">Custo Fixo:</span>
										<a class="kt-widget__data text-success">
										    {{number_format($p->custo_fixo, 2, ',', '.') }}
										</a>
									</div>

									<div class="kt-widget__info">
										<span class="kt-widget__label">Lucro Liquido:</span>
										<a class="kt-widget__data text-success">
										    {{number_format($p->vlucro_liquido, 2, ',', '.') }}
										</a>
									</div>
									...
									<div class="kt-widget__info">
										<span class="kt-widget__label">Lucro Liquido (%):</span>
										<a class="kt-widget__data text-success">
										   {{number_format($p->lucro_liquido, 2, ',', '.') }}
										</a>
									</div>
									...
									<div class="kt-widget__info">
										...
										<a class="kt-widget__data text-success">
											{{number_format(($p->valorDaUltimaCompra()/$p->conversao_unitaria), 2, ',', '.') }}
										</a>
									</div>
