        ...
        <div class="" id="kt_user_profile_aside" style="margin-left: 10px; margin-right: 10px;">
            ...
            <form method="get" action="/compras/pesquisa">
                <div class="row align-items-center">
                    ...
                    <input type="hidden" name="status_filtro" value="{{ isset($status_filtro) ? $status_filtro : 'ativas' }}">
                ...
                <div class="row align-items-center">
                    ...
                    <div class="form-group col-lg-2 col-md-4 col-sm-6">
                        ...
                        <select name="status_filtro" class="form-control">
                            <option value="ativas" {{ (isset($status_filtro) ? $status_filtro : 'ativas') == 'ativas' ? 'selected' : '' }}>Ativas</option>
                            <option value="canceladas" {{ (isset($status_filtro) ? $status_filtro : 'ativas') == 'canceladas' ? 'selected' : '' }}>Canceladas</option>
                            <option value="todas" {{ (isset($status_filtro) ? $status_filtro : 'ativas') == 'todas' ? 'selected' : '' }}>Todas</option>
                        </select>
                    </div>
            ...
            <label>Total de registros: {{ method_exists($compras, 'total') ? $compras->total() : count($compras) }}</label>
            <div class="row">
                @foreach($compras as $c)

                @php
                    $statusRaw = is_string($c->status) ? strtolower(trim($c->status)) : $c->status;
                    $cancelada = ($statusRaw === true || $statusRaw === 1 || $statusRaw === '1' || $statusRaw === 'true');
                    if(isset($status_filtro) && $status_filtro == 'canceladas'){
                        $cancelada = true;
                    }elseif(isset($status_filtro) && $status_filtro == 'ativas'){
                        $cancelada = false;
                    }
                @endphp
                    ...
                    <div class="card card-custom gutter-b example example-compact">
                        <div class="card-header">
                                        ...
                                        <ul class="navi navi-hover">
                                            ...
                                            <li class="navi-item">
                                                ...
                                                <span class="navi-link">
                                                    <span class="navi-text">
                                                        <span class="label label-xl label-inline label-light">Já cancelada</span>
                                                    </span>
                                                </span>
                                                @endif
                                            </li>
                            ...
                            <div class="card-body">
                                ...
                                <div class="kt-widget__info">
                                    <span class="kt-widget__label">Manifesto:</span>
                                    @if($c->dfe_id > 0)
                                    <a target="_blank" href="/dfe/{{$c->dfe_id}}" class="kt-widget__data text-success">{{ $c->dfe_id }}</a>
                                    @else
                                    <a class="kt-widget__data text-success">--</a>
                                    @endif
                                </div>
                                ...
                                <div class="kt-widget__info">
                                    ...
                                    @if($cancelada == true)
                                    <a class="kt-widget__data text-danger">Cancelada</a>
                                    @else
                                    <a class="kt-widget__data text-success">Ativa</a>
                                    @endif
                                </div>
