								...
								<div class="kt-section__body">
								    <div class="row">
									   <div class="form-group validated col-sm-6 col-lg-6">
										...
										<!--begin::Input-->
								       <select class="form-control select2 fornecedor" id="kt_select2_1" name="fornecedor_id" id = "fornecedor_id">
									      <option value="--">Selecione o fornecedor</option>
										     @foreach($fornecedores as $f)
										       <option value="{{$f->id}}"
											   @isset($conta)
											   @if($f->id == $conta->fornecedor_id)
													selected
													@endif
													@endisset
											   > 
												{{$f->razao_social}} ({{$f->cpf_cnpj}})</option>
								    ...
								    <div class="row">
										<div class="form-group validated col-sm-6 col-lg-6">
											<label class="col-form-label">Descrição da Conta</label>
											<div class="">
									...
									<div class="row">
									    ...
									    <div class="form-group col-lg-4 col-md-9 col-sm-12">
											...
											<div class="">
												<div class="input-group date">
													<input type="text" name="data_emissao" class="form-control @if($errors->has('emissao')) is-invalid @endif" readonly value="{{{ isset($conta) ? \Carbon\Carbon::parse($conta->data_emissao)->format('d/m/Y') : old('emissao') }}}" id="kt_datepicker_3" />
												...
												@if($errors->has('emissao'))
												<div class="center-align red lighten-2">
													<span class="white-text">{{ $errors->first('emissao') }}</span>
												</div>
												@endif
										...
										<div class="form-group validated col-lg-4 col-md-4 col-sm-6">
											<label class="col-form-label">Conta do DRE</label>

											<select class="custom-select form-control" id="contadre_id" name="contadre_id">
												@foreach($contasdre as $cat)
												<option value="{{$cat->id}}" @isset($conta)
													@if($cat->id == $conta->dreconta_id)
													selected
													@endif
													@endisset >{{$cat->nome}}
												</option>
