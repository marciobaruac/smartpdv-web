...
<div class="card card-custom gutter-b">
	<div class="card-body">
		<div class="" id="kt_user_profile_aside" style="margin-left: 10px; margin-right: 10px;">
			...
			<h3>Observação: <strong class="text-danger">R$ {{$agendamento->observacao}}</strong></h3>
			@endif

			@if($agendamento->valor_comissao > 0)
			<h3>Comissão: <strong class="text-danger">R$ {{$agendamento->valor_comissao}}</strong></h3>
			@endif
			...
			@else
			<h4>Agendamento finalizado em: <strong class="text-info">{{\Carbon\Carbon::parse($agendamento->updated_at)->format('d/m/Y H:i:s')}}</strong></h4>
			@endif
