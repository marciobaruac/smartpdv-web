...
class RelatorioController extends Controller
{
    ...
    public function filtroVendas(Request $request)
    {
        ...
        $vendas = Venda::select(\DB::raw('DATE_FORMAT(vendas.data_registro, "%d-%m-%Y") as data, sum(vendas.valor_total) as total, count(*) as total_vendas'))
        ...
        $vendasCaixa = VendaCaixa::select(\DB::raw('DATE_FORMAT(venda_caixas.data_registro, "%d-%m-%Y") as data, sum(venda_caixas.valor_total) as total, count(*) as total_vendas'))
            ->orWhere(function ($q) use ($data_inicial, $data_final) {
                if ($data_inicial && $data_final) {
    ...
    public function filtroVendaProdutos(Request $request)
    {
        ...
        $itensVenda = ItemVenda::select(\DB::raw('produtos.id as id, produtos.nome as nome, item_vendas.valor as valor_venda, sum(item_vendas.quantidade) as total, sum(item_vendas.quantidade * item_vendas.valor) as total_dinheiro'))
            ...
            ->groupBy('produtos.id', 'produtos.nome', 'item_vendas.valor')
        ...
        $itensVendaCaixa = ItemVendaCaixa::select([
            ...
            ->groupBy('produtos.id', 'produtos.nome', 'item_venda_caixas.valor')
            ->orderBy($ordem == 'nome' ? 'produtos.nome' : 'total', $ordem == 'nome' ? 'asc' : $ordem);
        ...
        usort($arr, function ($a, $b) use ($ordem) {
            if ($ordem == 'asc') {
                ...
                if ($a['nome'] == $b['nome']) {
                    return (float)$a['valor_venda'] <=> (float)$b['valor_venda'];
                }
    ...
    public function filtroVendaClientes(Request $request)
    {
        ...
        $vendasCaixa = VendaCaixa
            ...
            ->orWhere(function ($q) use ($data_inicial, $data_final) {
                if ($data_final && $data_final) {
                    return $q->whereBetween('venda_caixas.data_registro', [
                        $data_inicial,
                        $data_final
                    ]);
                }
    ...
    public function filtroVendaDiaria(Request $request)
    {
        ...
        $vendas = Venda
            ...
            ->Where(function ($q) use ($data_inicial, $data_final) {
                if ($data_final && $data_final) {
                    return $q->whereBetween('vendas.created_at', [
                        $data_inicial,
                        $data_final
                    ]);
                }
    ...
    private function uneArrayVendas($vendas, $vendasCaixa)
    {
        ...
        foreach ($vendasCaixa as $v) {
            ...
            if (isset($resultado[$data])) {
                $resultado[$data]['total'] += $v->total;
                $resultado[$data]['total_vendas'] += $v->total_vendas ?? 0;
            } else {
                $resultado[$data] = [
                    'data' => $data,
                    'total' => $v->total,
        ...
        return array_values($resultado); // Reindexa para garantir array sequencial
    }
    ...
    private function uneArrayProdutos($itemVenda, $itemVendasCaixa)
    {
        ...
        foreach ($itemVenda as $i) {
            $chave = $i->id . '-' . number_format((float)$i->valor_venda, 4, '.', '');
            ...
            array_push($adicionados, $chave);
            array_push($arr, $temp);
        }

        foreach ($itemVendasCaixa as $i) {
            $chave = $i->id . '-' . number_format((float)$i->valor_venda, 4, '.', '');
            if (!in_array($chave, $adicionados)) {
                $temp = [
                    'id' => $i->id,
                    'nome' => $i->nome,
                    'valor_venda' => $i->valor_venda,
                ...
                for ($aux = 0; $aux < count($arr); $aux++) {
                    $chaveArr = $arr[$aux]['id'] . '-' . number_format((float)$arr[$aux]['valor_venda'], 4, '.', '');
                    if ($chaveArr == $chave) {
                        $arr[$aux]['total'] += $i->total;
                        $arr[$aux]['total_dinheiro'] += $i->total_dinheiro;
                    }
                }
            }
        }
    ...
    public function cobrancaPendente(Request $request)
    {
        ...
        $contas = ContaReceber::whereBetween('data_vencimento', [
                $data_inicial,
                $data_final
            ])
            ->where('status', false);
    ...
    public function relatorioAtendimento(Request $request)
    {
        ...
        if ($data_final && $data_final) {
            $itens->whereBetween('item_pedidos.created_at', [
                $data_inicial,
                $data_final
            ]);
        }
            ...
            ->select(
                ...
                DB::raw("
                    COALESCE((SELECT SUM(ABS(m.quantidade))
                        ...
                        WHERE m.produto_id = p.id AND tipomov = 'saida' AND origem = 'pdv' AND DATE(m.created_at) BETWEEN '$startDate' AND '$endDate'), 0) as QtdVenda,

                    COALESCE((SELECT SUM(ABS(m.quantidade))
                        FROM estoque_movs m
                        JOIN estoquemovmanuals emm ON emm.estoquemov_id = m.id
                        JOIN estoque_apontamento_manuals ae ON ae.id = emm.estoquemanual_id
                        WHERE m.produto_id = p.id AND ae.Tipo = 'reducao' AND DATE(m.created_at) BETWEEN '$startDate' AND '$endDate'), 0) as QtdSaidaManual,
                    ...
                    COALESCE((SELECT SUM(ABS(m.quantidade))
                        FROM estoque_movs m
                        WHERE m.produto_id = p.id AND m.tipomov = 'entrada' AND DATE(m.created_at) BETWEEN '$startDate' AND '$endDate'
                        AND NOT EXISTS (
                            SELECT 1 FROM estoquemovmanuals emm
                            JOIN estoque_apontamento_manuals ae ON ae.id = emm.estoquemanual_id
                            WHERE emm.estoquemov_id = m.id
                        )
                        AND NOT EXISTS (
                            SELECT 1 FROM estoquemovcompras ec
                            WHERE ec.estoquemov_id = m.id
                        )
                    ), 0) as QtdCompra,
                    ...
                    COALESCE((SELECT SUM(ABS(m.quantidade))
                        ...
                        JOIN estoquemovmanuals emm ON emm.estoquemov_id = m.id
                        JOIN estoque_apontamento_manuals ae ON ae.id = emm.estoquemanual_id
                        WHERE m.produto_id = p.id AND ae.Tipo = 'incremento' AND DATE(m.created_at) BETWEEN '$startDate' AND '$endDate'), 0)
                    +
                    COALESCE((SELECT SUM(ABS(m.quantidade))
                        FROM estoque_movs m
                        JOIN estoquemovmanuals emm ON emm.estoquemov_id = m.id
                        JOIN estoque_apontamento_manuals ae ON ae.id = emm.estoquemanual_id
                        WHERE m.produto_id = p.id AND m.tipomov = 'entrada' AND ae.Tipo = 'pesoinicial' AND DATE(m.created_at) BETWEEN '$startDate' AND '$endDate'), 0)
                    as QtdEntradaManual,
                    ...
                    COALESCE((SELECT SUM(ABS(m.quantidade))
                        ...
                        JOIN estoquemovmanuals emm ON emm.estoquemov_id = m.id
                        JOIN estoque_apontamento_manuals ae ON ae.id = emm.estoquemanual_id
                        WHERE m.produto_id = p.id AND m.tipomov = 'saida' AND ae.Tipo = 'pesoinicial' AND DATE(m.created_at) BETWEEN '$startDate' AND '$endDate'), 0) as Sobra1,

                    COALESCE((SELECT SUM(ABS(m.quantidade))
                        FROM estoque_movs m
                        JOIN estoquemovmanuals em ON em.estoquemov_id = m.id
                        JOIN estoque_apontamento_manuals ae ON ae.id = em.estoquemanual_id
                        WHERE m.produto_id = p.id AND ae.Tipo = 'Saldo' AND m.quantidade > 0 AND DATE(m.created_at) BETWEEN '$startDate' AND '$endDate'), 0) as Falta,

                    COALESCE((SELECT SUM(ABS(m.quantidade))
                        FROM estoque_movs m
                        JOIN estoquemovmanuals em ON em.estoquemov_id = m.id
                        JOIN estoque_apontamento_manuals ae ON ae.id = em.estoquemanual_id
                        WHERE m.produto_id = p.id AND ae.Tipo = 'Saldo' AND m.quantidade < 0 AND DATE(m.created_at) BETWEEN '$startDate' AND '$endDate'), 0) as Sobra,
                    ...
                    (
                        ...
                        (
                            COALESCE((SELECT SUM(ABS(m.quantidade))
                                FROM estoque_movs m
                                JOIN estoquemovpdvs epdv ON epdv.estoquemov_id = m.id
                                WHERE m.produto_id = p.id AND tipomov = 'saida' AND origem = 'pdv' AND DATE(m.created_at) BETWEEN '$startDate' AND '$endDate'), 0)
                            +
                            COALESCE((SELECT SUM(ABS(m.quantidade))
                                FROM estoque_movs m
                                JOIN estoquemovmanuals emm ON emm.estoquemov_id = m.id
                                JOIN estoque_apontamento_manuals ae ON ae.id = emm.estoquemanual_id
                                WHERE m.produto_id = p.id AND ae.Tipo = 'reducao' AND DATE(m.created_at) BETWEEN '$startDate' AND '$endDate'), 0)
                            +
                            COALESCE((SELECT SUM(ABS(m.quantidade))
                                FROM estoque_movs m
                                JOIN estoquemovmanuals em ON em.estoquemov_id = m.id
                                JOIN estoque_apontamento_manuals ae ON ae.id = em.estoquemanual_id
                                WHERE m.produto_id = p.id AND ae.Tipo = 'Saldo' AND m.quantidade > 0 AND DATE(m.created_at) BETWEEN '$startDate' AND '$endDate'), 0)
                            +
                            COALESCE((SELECT SUM(ABS(m.quantidade))
                                FROM estoque_movs m
                                JOIN estoquemovmanuals em ON em.estoquemov_id = m.id
                            ...
                            COALESCE((SELECT SUM(ABS(m.quantidade))
                                ...
                                JOIN estoquemovmanuals emm ON emm.estoquemov_id = m.id
                                JOIN estoque_apontamento_manuals ae ON ae.id = emm.estoquemanual_id
                                WHERE m.produto_id = p.id AND m.tipomov = 'saida' AND ae.Tipo = 'pesoinicial' AND DATE(m.created_at) BETWEEN '$startDate' AND '$endDate'), 0)
                        )
                    ) AS Saldo
                ")
            )
