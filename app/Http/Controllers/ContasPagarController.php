...
class ContasPagarController extends Controller
{
	...
	public function index(){
		...
		$contasfixasagrupadas =  ContaPagar::
		...
		$contascomprasagrupadas =  ContaPagar::
		...
		$contasvariavelagrupadas =  ContaPagar::
		selectRaw(' sum(IF(valor_pago = 0 ,valor_integral,valor_pago)) as soma, dre_contas.nome')
		->join('dre_contas', 'dre_contas.id' , '=', 'conta_pagars.dreconta_id')
		->groupBy('dre_contas.nome')
		->orderBy('soma', 'desc')
		...
		->with('contas', $contas)
		->with('contasfixasagrupadas', $contasfixasagrupadas )
		->with('contascomprasagrupadas', $contascomprasagrupadas )
		->with('contasvariavelagrupadas', $contasvariavelagrupadas)
		
		->with('graficoJs', true)
		->with('contasPagarJs', true)
		//->with('somaContas', $somaContas)
		->with('infoDados', "Documentos Vencidos")
	...
	public function filtro(Request $request){
		...
		if(isset($fornecedor) && isset($dataInicial) && isset($dataFinal)){
			...
			$contasfixasagrupadas = ContaPagar::filtroDataFornecedorCustoFixo (
				$fornecedor, 
				$this->parseDate($dataInicial),
				$this->parseDate($dataFinal),
				$status
			...
			$contasvariavelagrupadas = ContaPagar::filtroDataFornecedorCustoVariavel (
				$fornecedor, 
				$this->parseDate($dataInicial),
				$this->parseDate($dataFinal),
				$status
			...
			$contascomprasagrupadas = ContaPagar::filtroDataFornecedorCompra (
				$fornecedor, 
				$this->parseDate($dataInicial),
				$this->parseDate($dataFinal),
				$status
			...
			$contasfixasagrupadas =  ContaPagar::filtroDataContaDre($this->parseDate($dataInicial),
			$this->parseDate($dataFinal),
			$status);

			$contascomprasagrupadas =  ContaPagar::filtroDataContaCompra($this->parseDate($dataInicial),
			$this->parseDate($dataFinal),
			$status);
		
			$contasvariavelagrupadas =  ContaPagar::filtroDataContaVariavel($this->parseDate($dataInicial),
			$this->parseDate($dataFinal),
			$status);
			...
			$contasfixasagrupadas = ContaPagar::filtroFornecedorCustoFixo(
				$fornecedor,
				$status
			);

			$contasvariavelagrupadas = ContaPagar::filtroFornecedorCustoVariavel($fornecedor,
			$status);

			$contascomprasagrupadas = ContaPagar::filtroFornecedorCompra($fornecedor,$status);


		}else{
			$contas = ContaPagar::filtroStatus($status);

			$contasfixasagrupadas = ContaPagar::filtroStatusCustoFixo($status);
			$contasvariavelagrupadas = ContaPagar::filtroStatusCustoVariavel($status);

			$contascomprasagrupadas = ContaPagar::filtroStatusCompra($status);
		}

	//	$somaContas = $this->somaCategoriaDeContas($contas);
		//return Excel::download( $contas, 'users.csv');
		return view('contaPagar/list')
		->with('contas', $contas)
		->with('contasfixasagrupadas',$contasfixasagrupadas)
		->with('contascomprasagrupadas', $contascomprasagrupadas )
