...
class VendaCaixaController extends Controller
{
  ...
  public function save(Request $request)
  {
    ...
    if ($venda['tipo_pagamento'] == '02' || $venda['tipo_pagamento'] == '08' || $venda['tipo_pagamento'] == '09') {
    ...
    if ($venda['tipo_pagamento'] == '03' || $venda['tipo_pagamento'] == '07') {
    ...
    $result = VendaCaixa::create([
      ...
      'intencao_venda_id'             => isset($venda['dados_tef']) ? $venda['dados_tef']['intencao_venda_id'] : 0
        ...
        if (isset($venda['pizza']) && $i['pizza'] == 1) {
          ...
          foreach ($sabores as $sb) {
            ...
            if (!empty($produto->receita)) {
              ...
              foreach ($receita->itens as $rec) {
                ...
                $stockMove->downStock(
                  $rec->produto_id,
                  (float) str_replace(",", ".", $i['quantidade']) * ((($rec->quantidade / $totalSabores) / $receita->pedacos) * $produtoPizza->tamanho->pedacos) / $receita->rendimento
                );
              }
            }
          }
          ...
          foreach ($receita->itens as $rec) {
            ...
            if (!empty($rec->produto->receita)) {
              ...
              // die();
              $produtor = Produto::where('id', $rec->produto_id)
                ->first();
  ...
  {
    return ConfigCartao::where('cartao', $cartao)
