...
class ProductController extends Controller
{
    ...
    public function importacao(){
		...
		->with('title', 'Importação de Produtos');
    }
    ...
    private function setaEstoque($produtos){
        ...
        foreach($produtos as $p){
            ...
            $valorvenda = $p->valorTabelaPadrao();
            ...
            if ($valorvenda == 0){
                ...
                $custoproduto = $p->custoprodutofabricado($p->id);
                $p->lucro_bruto = ( $valorvenda- $custoproduto)/  $valorvenda*100;                 
                $p->custo_fixo  = ($valorvenda * $config->custo_fixo)/100 ;
                $p->vlucro_liquido = $valorvenda- ($custoproduto+ $p->custo_fixo );
                $p->lucro_liquido = $p->vlucro_liquido / $valorvenda*100;
    ...
    public function new(){
        ...
        $natureza = Produto::NaturezaPadrao($config->nat_op_padrao);
    ...
    public function save(Request $request){
        ...
        $nomeImagem = $this->salveImagemProduto($request, $produto); // salva a imagem no produto comum
        
        // Salva produto na tabela de preço padrão
        
       
        $this-> insereListapreco($result->id, str_replace(",", ".", $request->input('valor_venda')));
    ...
    private function insereListapreco($produtoid, $valorvenda){
        ...
        ProdutoListaPreco ::create($produtolistapreco);
    ...
    public function filtroCategoria(Request $request){
        ...
        if($porCodigoBarras == 1){
            $query = Produto::where('codBarras', $pesquisa)->where('ativo',true);
        }else{
            $query = Produto::where('nome', 'LIKE', "%$pesquisa%")->where('ativo',true);
        }

        if($categoria != '-'){
            $query = Produto::where('categoria_id', $categoria)->where('ativo',true);
        }
    ...
    public function receita($id){
        ...
        $produtos = Produto::all()->where('ativo',true);
    ...
    public function update(Request $request){
        ...
        $resp->referencia2 = $request->referencia2 ?? '';
        $resp->referencia3 = $request->referencia3 ?? '';
        $resp->referencia4 = $request->referencia4 ?? '';
        ...
        $this->updateProdutoListapreco($request, $resp);
        if($result){
            if($request->atribuir_delivery){
       ...
       //return redirect('/produtos/filtroCategoria');
    ...
    public function delete($id){
        try{
            ...
            if($salvar){
                session()->flash('mensagem_sucesso', 'Produto Inativado!');
            }else{
    ...
    public function all(){
        $products = Produto::all()->where('ativo',true);
        $arr = array();
    ...
    public function getProdutoTabela($id){
        $produto = Produto::
        where('id', $id)
        ->first();
        if($produto->delivery){
       ...
       if ($preco){
        ...
        $produto->valor_venda = $valorvenda; 
    ...
    public function updatecustoproduto($id,$custoproduto){
        $product = new Produto();
        $up    = $product  ->where('id', $id)->first(); 

        $up->valor_compra = $custoproduto;
    ...
    public function salvarProdutoDaNota(Request $request){
        ...
        if  ($produto['CST_COFINS'] == '04'){
            $pcofins =  0;
        }
        else{
            $pcofins =  $tributacao->cofins;
       
        }

        if  ($produto['CST_CSOSN'] == '60'){
            $picms =  0;
            $pcfop = '5405';
            $pcfopfora  = '6405';
        }
        else{
            $picms =  $tributacao->icms;
            $pcfop = '5102';
            $pcfopfora  = '6102';
        ...
        $result = Produto::create([
            ...
            'perc_cofins' => $pcofins,
            'perc_ipi' => 0,
            'CFOP_saida_estadual' =>  $pcfop,
            'CFOP_saida_inter_estadual' =>  $pcfopfora,
            'codigo_anp' => '', 
            'descricao_anp' => '',
            'cListServ' => '',
            'imagem' => '',
        ...
        $this->insereListapreco($result->id,$valorVenda);
    ...
    public function salvarEdicaoProduto(Request $request){
      ...
      $resp   = $produtoListapreco  ->where('produto_id',  $id)->where('lista_id', 0)->first(); 
      
     
     if ($resp) {
        $resp->valor = $valorVenda;

        $result = $resp->save();
  
     }else{

        $this->insereListapreco($id ,$valorVenda);
    ...
    private function updateProdutoListapreco($request, $produto){
        // $this->_validateDelivery($request);
     

        $produtoListapreco = new ProdutoListaPreco();
        $up    = $produtoListapreco  ->where('produto_id', $produto->id)->where('lista_id', 0)->first(); 
          if($up){
           
            $up->valor   =  str_replace(",", ".", $request->input('valor_venda'));
            ...
            $produtolistaprecod = [
                'lista_id'   => 0,
                'produto_id' => $produto->id,
                'valor'      =>  str_replace(",", ".", $request->input('valor_venda')),
                'percentual_lucro' =>  0,
                'quantidade_minima' => 1
            
            ];
    
            ProdutoListaPreco ::create($produtolistaprecod);
        }
    ...
    {
        Excel::import(new ProdutosImport, $request->file('file')->store('temp'));
        return back();
    }
