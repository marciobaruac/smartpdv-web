...
class ContaReceberController extends Controller
{
	...
	public function index(){
		...
		//	date('Y-m-d', strtotime('+1 month'))])->where('status', '<>', 2)
		//->get();
		$contas = ContaReceber::
		where('ativo',true)
		
		->whereBetween('data_recebimento', [date("Y-m-d"), 
		date('Y-m-d')])->where('status', '<>', 2)
		->get();
	...
	public function update(Request $request){
		...
		$conta->data_recebimento  = $this->parseDate($request->vencimento);
	...
	public function receberConta(Request $request){
		...
		if($conta->valor_integral != $request->valor){
			$valor = $request->valor;
			$datarecebimento =  $this->parseDate($request->data_pagamento);
			...
			if($conta->venda_id != null){
				...
				->with('datarecebimento',$datarecebimento)
				->with('title', 'Receber Conta');
			}else{
			...
			$conta->data_recebimento = $this->parseDate($request->data_pagamento);
	...
	public function receberSomente(Request $request){
		...
		if($result){
			...
			session()->flash('mensagem_sucesso', 'Conta recebida Receber!');
		}else{
	...
	public function cancelar(Request $request){
		...
		if($result){
			...
			session()->flash('mensagem_sucesso', 'Registro Cancelado com Sucesso!');
		}else{
