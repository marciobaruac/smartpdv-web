...
class DFeController extends Controller
{
	...
	public function buscarIntegranotas(Request $request)
	{
		\Log::info('[IntegraNotas] Chamou buscarIntegranotas', [
			'dfe_controller' => __FILE__,
			'integra_file' => base_path('app/Http/Controllers/IntegraNotasController.php'),
			'integra_file_exists' => file_exists(base_path('app/Http/Controllers/IntegraNotasController.php')),
			'integra_class_exists' => class_exists(\App\Http\Controllers\IntegraNotasController::class),
		]);

		$controller = new IntegraNotasController();
		return $controller->importarNotasEntradaWeb($request);
	}
