...
//use App\Models\Helpers\StockMove;
...
class CompraFiscalController extends Controller
{
	...
	public function salvarItem(Request $request){
		...
		str_replace(",", ".", $prod['quantidade'] * $produtoBD->conversao_unitaria),str_replace(",", ".", $prod['valor']),'Compra','Obs. '. 'Compra'.'-'.  'Mov: '.$result->compra_id,$result->id);
        
		$produtoBD->updatecustoproduto($prod['produto_id'], str_replace(",", ".", $prod['valor']));
		...
		try{
			...
			foreach($itens as $i){
				...
				if ($i['conversao_unitaria'] == 1){
					...
					$stockMove->pluStock((int) $itemRes->produto_id, 
					str_replace(",", ".", $i['qCom'][0]),$custoproduto,'Compra','Obs. '. 'NFE: '.$nf['nNf'],$itemRes->id);
					...
					$stockMove->pluStock((int) $itemRes->produto_id, 
					str_replace(",", ".", $i['qCom'][0]*$i['conversao_unitaria']),$custoproduto,'Compra','Obs. '. 'NFE: '.$nf['nNf'],$itemRes->id);
