...
class ListaPrecoController extends Controller
{
    ...
    public function newprodutotabela($id){
        ...
        ->with('title', 'Cadastrar produto na Tabela');
    }
	...
	public function saveprodutotabela(Request $request){
	   $this->_validateproduto($request);



		$produtolistaprecod = [
			'lista_id'   => $request->input('listaid'),
			'produto_id' => $request->input('produto'),
			'valor'      =>  str_replace(",", ".", $request->input('novo_valor')),
			'percentual_lucro' =>  0,
			'quantidade_minima' => str_replace(",", ".", $request->input('quantidade_minima')),
			'referencia' => str_replace(",", ".", $request->input('referencia'))
		
		];
		
	
		$result = ProdutoListaPreco ::create($produtolistaprecod);
		...
		return redirect('/listaDePrecos/ver/' . $result->lista_id);
	...
	private function _validateproduto(Request $request){
		$rules = [
			'novo_valor' => 'required',
		...
		$messages = [
			'novo_valor.required' => 'O campo obrigatório.',
		//	'nome.max' => '40 caracteres maximos permitidos.',
			'quantidade_minima.required' => 'Campo obrigatório.'
