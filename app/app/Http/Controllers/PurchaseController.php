...
class PurchaseController extends Controller
{
    ...
    public function index(Request $request){
        $statusFiltro = $this->normalizarFiltroStatus($request->status_filtro);
        $compras = Compra::orderBy('id', 'desc');
        $this->aplicarFiltroStatus($compras, $statusFiltro);
        $compras = $compras->paginate(15);

        $somaCompraMensal = $this->somaCompraMensal();
        return view('compraManual/listAll')
        ->with('compras', $compras)
        ->with('status_filtro', $statusFiltro)
    ...
    public function filtro(Request $request){
        ...
        if(($dataInicial != null && $dataFinal == null) || ($dataInicial == null && $dataFinal != null)){
            session()->flash('mensagem_erro', 'Informe o período completo para filtrar por data!');
            return redirect('/compras?status_filtro=' . $statusFiltro);
        }
        ...
        if(isset($fornecedor) && trim($fornecedor) != ''){
            $query->where('fornecedors.razao_social', 'LIKE', "%$fornecedor%");
        }

        if(isset($dataInicial) && isset($dataFinal) && $dataInicial != '' && $dataFinal != ''){
            $query->whereBetween('compras.created_at', [$this->parseDate($dataInicial), $this->parseDate($dataFinal, true)]);
            $diferencaDatas = $this->diferencaEntreDatas($this->parseDate($dataInicial), $this->parseDate($dataFinal));
        }

        $this->aplicarFiltroStatus($query, $statusFiltro);
        $compras = $query->orderBy('compras.id', 'desc')->get();
    ...
    private function normalizarFiltroStatus($statusFiltro){
        if($statusFiltro == 'canceladas' || $statusFiltro == 'todas' || $statusFiltro == 'ativas'){
            return $statusFiltro;
        }
        return 'ativas';
    }

    private function aplicarFiltroStatus($query, $statusFiltro){
        if($statusFiltro == 'todas') return;

        if($statusFiltro == 'canceladas'){
            $query->where(function($q){
                $q->where('compras.status', true)
                ->orWhere('compras.status', 1)
                ->orWhere('compras.status', 'true');
            });
            return;
        }

        $query->where(function($q){
            $q->where('compras.status', false)
            ->orWhere('compras.status', 0)
            ->orWhere('compras.status', 'false')
            ->orWhereNull('compras.status');
        });
    }
    ...
    public function delete($id){
        ...
        // baixa de estoque
            ...
            $stockMove->downStock($i->produto->id,($i->quantidade * $i->produto->conversao_unitaria), $i->produto->valor_compra, 'Extorno da Entrada','Obs. '. 'N: '.'-'.  'Mov: '. $compra->id,  $i->id );
        ...
        $compra->status = true; 
        if($compra->save()){
            session()->flash('mensagem_sucesso', 'Registro Cancelado!');
            DB::commit();
        }else{
