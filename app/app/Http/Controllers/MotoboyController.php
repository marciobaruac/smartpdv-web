...
class MotoboyController extends Controller
{
	...
	public function novoPedido(){
		...
		return view('motoboys/novo_pedido')
		->with('motoboys', $motoboys)
		->with('title', 'Novo pedido motoboy');
	}

	public function savePedido(Request $request){
		try{
			PedidoMotoboy::create([
				'motoboy_id' => $request->motoboy,
				'pedido_id' => NULL,
				'valor' => $request->valor,
				'status' => 0,
			...
			session()->flash('mensagem_sucesso', 'Entrega adicionada com sucesso!');
		}catch(\Exception $e){
			session()->flash('mensagem_erro', 'Erro ao adicionar!');
		}
		return redirect('/motoboys/entregas');
	}
