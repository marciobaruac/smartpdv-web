...
class OrcamentoController extends Controller
{
	...
	public function index(){
		...
		$orcamentos = $this->setaProdutoNaoCadastro($orcamentos);
	...
	public function gerarVenda(Request $request){
		...
		foreach ($orcamento->itens as $i) {
			...
			$custoproduto = 0; 
			$custoproduto = (float) $i->quantidade * $prod->valor_compra;
			...
			if(!empty($prod->receita)){
				...
				$stockMove->downStock((int) $i->produto_id ,  $i->quantidade , $prod->valor_compra ,'Venda','Obs. '. 'N: '.'-'.  'Mov: '. $result->id, $resultvendaitem->id);
		...
		foreach ($orcamento->duplicatas as $key => $f) {
			...
			$resultFatura = ContaReceber::create([
				...
				'valor_recebido' => $f->valor,
				'status' => true,
				'referencia' => "Parcela, ".($key+1).", da Venda " . $result->id,
				'categoria_id' => 2,
				'forma_recebimento' => $orcamento->tipo_pagamento,
				'usuario_id' => get_id_user()
			]);
		}
	...
	private function setaProdutoNaoCadastro($orcamentos){
        
        
      

        foreach($orcamentos as $o){
            $temproduto = ItemOrcamento::where('orcamento_id', $o->id)
			->where('produto_id', '= 0')
			->first();
            $o->temprodutonaocadastrado = $temproduto == null ? false : true;
