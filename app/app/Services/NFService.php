...
class NFService{
	...
	public function gerarNFe($idVenda,$nfnumero){
		$venda = Venda::
		where('id', $idVenda)
		->first();
	...
	//	$stdIde->nNF = (int) $venda->NfNumero;
		...
		$stdEnderDest->xMun = $this->retiraAcentos(strtoupper($venda->cliente->cidade->nome));
		...
		foreach($venda->itens as $i){
            ...
            if (!$venda->natureza->simplesremessa){
		...
		//TAG IMPOSTO
			...
			if($i->observacao != ""){
				...
				$std->infAdProd = $this->retiraAcentos($i->observacao);
	...
	//Fatura
	if($somaISS == 0 && $venda->natureza->simplesremessa= 0){
		$stdFat = new \stdClass();
		$stdFat->nFat = (int)$lastNumero+1;
		$stdFat->vOrig = $this->format($somaProdutos);
		$stdFat->vDesc = $this->format($venda->desconto);
	...
	//Duplicata
		...
		if ($venda->tipo_pagamento =='04') {

			$stdDetPag->tPag = '05';
		
		}

		else if ($venda->tipo_pagamento !='06') {
