...
class Produto extends Model
{
	protected $fillable = [
		...
		'cBenef', 'CEST','ativo', 'referencia2','referencia3','referencia4'
	];
	...
	public function listaPrecoPadrao(){
		return $this->hasMany('App\Models\ProdutoListaPreco', ['produto_id', 'id'],['lista_id', 0]);
	}


	public static function verificaCadastrado($ean, $nome, $referencia,$consultaprodutocadastro){
		$result = null;
		
		if ($consultaprodutocadastro==0){
	...
	public static function NaturezaPadrao($padrao){
	...
	public function valorTabelaPadrao(){
	...
	public function custoprodutofabricado($codigo){
		...
		if ($this->composto == 0) {
			...
			$custo = Receita::
			selectRaw('Round(sum(item_receitas.quantidade*produtos.valor_compra),2) as valor')

			->join('item_receitas', 'item_receitas.receita_id' , '=', 'receitas.id') 

			 ->join('produtos', 'produtos.id' , '=', 'item_receitas.produto_id')

			->where('receitas.produto_id', $codigo)
