...
$('#kt_select2_3').change(() => {
    ...
    CLIENTES.map((d) => {
        if (d.id == id) {
          ...
          //    $('#modalclientedestino').modal('show')
...
$('#addProdnaocadastro').click(() => {

    $('#modalprodutonaocadastrado').modal('show')

    $('#modalprodutonaocadastrado').on('shown.bs.modal', function(event) {
        $("#nomeproduto").focus();
      })
...
function addItemTablenao(codigo, nome, quantidade, valor, custo) {
   limparDadosFatura();
   TOTAL += parseFloat(valor.replace(',', '.')) * (quantidade.replace(',', '.'));
   TOTAL = parseFloat(TOTAL.toFixed(2));
...
$('#tipoPagamento').change(() => {
    ...
    data = $('#kt_datepicker_1').val();
    let data30aux = $('#kt_datepicker_1').val();
    dat1 = $('#kt_datepicker_1').val();   
    //date = new Date(new Date(data).setDate(new Date(data).getDate() + 30));
    //date = new Date(2022/04/28);
    
    var datames = convertDataMes(data);

    let now30  =  new Date(new Date(datames).setDate(new Date(datames).getDate() + 30))

    let now1  =  new Date(new Date(datames).setDate(new Date(datames).getDate() + 1))

     //let data30 = date.setDate(date.getDate() + 30);
   //  let data30 = (date.getDate() < 10 ? "0" + date.getDate() : date.getDate()) +
  ////   "/" + ((date.getMonth() + 1) < 10 ? "0" + (date.getMonth() + 1) : (date.getMonth() + 1)) +
   // "/" + date.getFullYear();

    let data30 = (now30.getDate() < 10 ? "0" + now30.getDate() : now30.getDate()) +
    "/" + ((now30.getMonth() + 1) < 10 ? "0" + (now30.getMonth() + 1) : (now30.getMonth() + 1)) +
   "/" + now30.getFullYear();
   ...
   // let data1 = (date1.getDate() < 10 ? "0" + date1.getDate() : date1.getDate()) +
    ...
    let data1 = (now1.getDate() < 10 ? "0" + now1.getDate() : now1.getDate()) +
...
function salvarVenda(btnClick) {
    ...
    ITENS.map((v) => {
        if (v.codigo == 0) {
            swal('Erro', 'Não pode realizar venda, produto não cadastrado!', 'error')
     ...
     verificaLimite((limite) => {
        ...
        if ((limite + TOTAL) <= CLIENTE.limite_venda ||
            $('#tipoPagamento').val() != 'conta_crediario') {
...
$('#incluirproduto').click(() => {
    ...
    let codigo = 0;
    let quantidade = $('#quantidadenao').val();  
    let nome =  $('#nomeproduto').val();
    let valor = $('#valornao').val();
       
    addItemTablenao(codigo, nome, quantidade, valor, 1);
    
  

    $('#modalprodutonaocadastrado').modal('hide');
