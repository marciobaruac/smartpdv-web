...
function modalOpen(){
	$('.novo-cliente').css('display', 'block')
	$('.cliente-cad').css('display', 'none')
...
$('#btn-send-cliente').click(() => {
	...
	.done((res) => {
		...
		.then(() => {
			// location.reload();
			$('.novo-cliente').css('display', 'none')
			$('.cliente-cad').css('display', 'block')
			$('#cli_novo').val(res.razao_social)
			$('#cli_novo_id').val(res.id)
...
$('#btn-send').click(() => {
	...
	let cli_novo_id = $('#cli_novo_id').val()	
	...
	getServicosParaSalvar((itens) => {
		...
		if(cliente_id == 'null' && cli_novo_id == 0){
			msg += "\nSelecione o cliente"
		}
