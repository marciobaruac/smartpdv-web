	...
	if(cont > 1){
		...
		if(cont > 0){
			...
			({
				...
				url: path + 'notacomplementar/enviarSefaz',
				dataType: 'json',
				success: function(e){
					...
					if(retorno == 'Erro'){
						...
						swal("Sucesso", "Nota Complementar emitida com sucesso RECIBO: "+recibo, "success")
						.then(() => {
							window.open(path+"notacomplementar/imprimir/"+id, "_blank");
							location.reload()
						})
					}
...
function cancelar(){
	...
	if(cont > 1){
		...
		({
			...
			url: path + 'notacomplementar/cancelar',
			dataType: 'json',
			success: function(e){
				$('#modal1').modal('hide');
				console.log(e)
...
function xmlTemp(){
	...
	if(cont > 1){
		...
		if(id > 0){
			window.open(path+"notacomplementar/xmltemp/"+id, "_blank");
		}else{
