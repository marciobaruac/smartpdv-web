...
function montaHtml(obj){
	...
	obj.map((v) => {
		...
		criaDiv(v.comanda, nome, v.quantidade, v.data, v.id, 
			v.adicionais, v.saboresPizza, v.tamanhoPizza, v.pedido_id, v.observacao, v.cor, v.mesa, v.complementos, (res) => {
...
function criaDiv(comanda, nome, quantidade, data, item_id, adicionais, 
	saboresPizza, tamanhoPizza, pedidoId, obs, cor, mesa, complementos, call){

	let tipo = comanda != null ? 'Comanda' : 'Item Delivery Pedido <strong class="text-danger">' + pedidoId + '</strong>';
	...
	if(mesa != ""){
		html += '<h3 style="margin-top: 10px;"> <strong class="text-info">'+ mesa+
	'</strong></h3>';
	}
	...
	'<h6>Complementos: <strong>' + complementos + '</strong></h6>'+
