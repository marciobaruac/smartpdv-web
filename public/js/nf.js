...
function notacomplementar() {
    ...
    if (cont > 1) {
        ...
        window.open(path + "nf/renderizarXmlcomplementar/" + id, "_blank");
    }
}
...
function inutilizarnfce() {

    let justificativa = $('#justificativa_inutnfce').val();
    let nInicio = $('#nInicionfce').val();
    let nFinal = $('#nFinalnfce').val();

    // $('#preloader3').css('display', 'block');
    $('#btn-inut-3').addClass('spinner')
            ...
            if (retorno == 'Erro') {
                ...
                swal("Sucesso", "NFCe gerada com sucesso RECIBO: " + recibo, "success")
                    .then(() => {
                        ...
                        window.open(path + 'nfce/imprimirvenda/' + vendaId, '_blank');
...
function modalCancelar(id) {
    $('#modalcancelarnfce').modal('show');
    $('#venda_id').val(id)
}



function modalinfnfe(id) {
    $('#modalinf').modal('show');
    $('#venda_id').val(id)
}
...
function cancelarnfce() {
    ...
    let justificativa = $('#justificativanfce').val();
    let id = $('#venda_id').val();
    let token = $('#_token').val();
    $.ajax({
        type: 'POST',
...
function atualizarinfvenda() {


    let infnfe = $('#infnfe').val();
    let id = $('#venda_id').val();
    let token = $('#_token').val();
    $.ajax({
        type: 'POST',
        ...
        error: function (e) {
            swal("Erro", 'Erro na atualização', "warning")
